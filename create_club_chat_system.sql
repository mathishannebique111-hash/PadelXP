...
-- =============================================
-- Reproduction exacte du système joueur-admin mais pour les clubs

-- =============================================
-- TABLE 1 : CLUB_CONVERSATIONS (Une par club)
-- =============================================
...
-- =============================================
-- TABLE 2 : CLUB_MESSAGES (Contenu des échanges)
-- =============================================
...
-- CLUBS : Voir uniquement leur conversation
-- Comme "Joueurs voient leur conversation" mais adapté pour les clubs
...
-- CLUBS : Voir uniquement leurs messages
CREATE POLICY "Clubs voient leurs messages"
ON club_messages FOR SELECT
TO authenticated
USING (
  EXISTS (
    SELECT 1 FROM club_conversations cc
    WHERE cc.id = club_messages.conversation_id
    AND (
      EXISTS (
        SELECT 1 FROM profiles 
        WHERE profiles.id = auth.uid() 
        AND profiles.club_id IS NOT NULL
        AND profiles.club_id::uuid = cc.club_id
      )
      OR EXISTS (
        SELECT 1 FROM club_admins ca
        INNER JOIN clubs c ON (c.id::text = ca.club_id OR c.slug = ca.club_id)
