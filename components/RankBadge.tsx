...
export default function RankBadge({ rank, size = "md", className = "", isClub = false }: RankBadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center justify-center rounded-full font-bold border-2 backdrop-blur-sm relative overflow-hidden transition-all duration-300 hover:scale-105 ${sizeClasses[size]} ${config.gradient} ${config.border} ${config.glow} ${rank > 3 && isClub ? '!text-white' : config.textColor} ${className}`}
      style={{
        ...config.style,
