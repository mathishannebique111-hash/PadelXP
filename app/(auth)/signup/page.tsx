...
function SignupForm() {
  ...
  const onEmailSignup = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/clubs/signup", {
        ...
        credentials: "include",
        body: JSON.stringify({ email, password, firstName, lastName, promoCode: promoCode.trim() || undefined }),
      });
  ...
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-center text-white px-6 overflow-hidden">
      ...
      {/* Logo PadelXP (dans le flux pour éviter le chevauchement) */}
      <div className="relative z-20 mb-6 flex justify-center pointer-events-none animate-fadeIn">
        <img
      ...
      <div className="relative z-[50] w-full max-w-sm rounded-2xl bg-white/10 border border-white/20 backdrop-blur-md shadow-xl p-6 animate-fadeIn">
        <div className="text-sm uppercase tracking-[0.3em] text-white/40 mb-2 text-center">Inscription Club</div>
        ...
        <form onSubmit={onEmailSignup} className="space-y-3">
          ...
          <div className="space-y-0.5">
            <label className="text-[10px] text-white/50 uppercase tracking-wider ml-1">Code Partenaire (optionnel)</label>
            <input type="text" className="w-full rounded-lg bg-white/5 border border-white/10 px-3 py-2 text-sm text-white placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-[#0066FF]" value={promoCode} onChange={(e) => setPromoCode(e.target.value.toUpperCase())} />
          </div>
          ...
          <div className="space-y-0.5">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute inset-y-0 right-2 my-1 px-1.5 text-[10px] font-semibold text-white/70 hover:text-white rounded-md bg-white/5 hover:bg-white/10"
              >
          ...
          <div className="pt-1">
            <label className="flex items-start gap-2 text-[11px] text-white/60 cursor-pointer select-none">
              <input type="checkbox" className="mt-0.5 w-3.5 h-3.5 rounded border-white/10 bg-white/5 text-[#0066FF] focus:ring-[#0066FF]" checked={accept} onChange={(e) => setAccept(e.target.checked)} />
          ...
          <div className="pt-3">
            <button
              disabled={loading}
              className="w-full py-3 rounded-xl text-white font-bold text-sm transition-all hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_0_20px_rgba(0,102,255,0.4)]"
        ...
        <div className="mt-6 pt-4 border-t border-white/5 text-center">
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-1 text-[9px] text-white/30 uppercase tracking-wide">
            <Link href="/legal?returnTo=/signup" className="hover:text-white transition-colors">Mentions</Link>
