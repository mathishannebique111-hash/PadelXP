...
export default function TodayMatchesList() {
    ...
    useEffect(() => {
        async function fetchMatches() {
            setIsLoading(true);
            try {
                const response = await fetch(`/api/admin/matches/today?date=${selectedDate}`);
                if (response.ok) {
    ...
    const displayDate = isToday ? "aujourd'hui" : `le ${new Date(selectedDate).toLocaleDateString('fr-FR')}`;
        ...
        <div className="mt-8 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                    ...
                    Matchs {isToday ? "d'aujourd'hui" : `du ${new Date(selectedDate).toLocaleDateString('fr-FR')}`}
                </h2>
                
                <div className="flex items-center gap-2 bg-slate-800/50 rounded-xl px-4 py-2 border border-white/5">
                    <Calendar className="w-4 h-4 text-slate-400" />
                    <input 
                        type="date"
                        value={selectedDate}
                        onChange={(e) => setSelectedDate(e.target.value)}
                        className="bg-transparent border-none text-white text-sm outline-none focus:ring-0 [&::-webkit-calendar-picker-indicator]:filter [&::-webkit-calendar-picker-indicator]:invert cursor-pointer"
            ...
            {isLoading ? (
                ...
                <div className="mt-8 p-12 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-white/5 text-center">
                    ...
                    <p className="text-slate-400">Aucun match n'a été enregistré {displayDate}.</p>
                </div>
            ) : (
