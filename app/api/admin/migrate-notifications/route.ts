...
// Secret pour protéger l'endpoint (à définir dans .env.local)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { secret, clearExisting = false } = body;
    ...
    if (clearExisting) {
      const { error: deleteError } = await supabase
        .from("notifications")
        .delete()
        .in("type", ["level_up", "badge_unlocked", "top3_ranking"]);

      if (deleteError) {
        logs.push(`⚠️ Erreur suppression notifications: ${deleteError.message}`);
      } else {
        logs.push("✅ Notifications existantes supprimées");
      }
    }

    // 3. Récupérer tous les clubs actifs
    ...
    if (clubsError) {
      return NextResponse.json(
        { error: "Erreur récupération clubs", details: clubsError },
        { status: 500 }
      );
    }

    logs.push(`📊 ${clubs?.length || 0} clubs trouvés`);

    let totalNotificationsCreated = 0;
    let totalPlayersProcessed = 0;

    // 4. Pour chaque club, traiter les joueurs
    for (const club of clubs || []) {
      ...
      if (!players || players.length === 0) {
        logs.push(`   ⚠️ Aucun joueur`);
        continue;
      }

      logs.push(`   👥 ${players.length} joueurs`);
      ...
      // 5. Créer les notifications pour chaque joueur
      for (let i = 0; i < leaderboard.length; i++) {
        ...
        if (notificationsToCreate.length > 0) {
          ...
          if (!insertError) {
            ...
            logs.push(
              `   ✅ ${player.player_name}: ${notificationsToCreate.length} notifications`
            );
          }
        }
    ...
    logs.push(`\n✅ Migration terminée!`);
    logs.push(`📊 ${totalPlayersProcessed} joueurs, ${totalNotificationsCreated} notifications`);

    return NextResponse.json({
      success: true,
      stats: {
        players: totalPlayersProcessed,
        notifications: totalNotificationsCreated,
      },
      logs,
    });
  } catch (error: any) {
