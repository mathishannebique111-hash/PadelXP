  ...
  useEffect(() => {
    (async () => {
      ...
      if (profile.club_slug !== slug) {
        // tenter via club_id
        const { data: club } = await supabase.from("clubs").select("slug").eq("id", profile.club_id).single();
        if (club?.slug !== slug) { setClubOk(false); return; }
      }
  ...
  return (
    <div className="min-h-screen bg-black text-white px-6 py-10">
      <div className="max-w-6xl mx-auto">
        ...
        {!clubOk && <p className="text-red-400 text-sm mb-4">Vous n'êtes pas rattaché à ce club.</p>}
