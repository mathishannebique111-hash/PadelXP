    ...
    <div className="space-y-8">
      ...
      {top3.length > 0 && (
        <div className="mb-8">
          <div className="mb-4 flex items-center justify-center gap-3">
            ...
            <span className="inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-sm font-semibold text-white shadow-sm">
          ...
          <div className="md:hidden space-y-4 mt-8">
            {top3.map((player, index) => {
              ...
              return (
                <div
                  key={player.user_id}
                  className={`rounded-2xl p-5 relative overflow-hidden shadow-2xl border-4 ${
                ...
                >
                  <div className="absolute top-2 right-2 z-20 drop-shadow text-5xl">
                    {icons[index]}
                  </div>
                  {index === 0 && (
                    <div className="absolute -top-4 -left-4 z-20">
                      <span className="inline-flex items-center rounded-full bg-yellow-100 text-yellow-800 px-2 py-0.5 text-[11px] font-semibold shadow-sm border border-yellow-300">
                        Meilleur joueur
                      </span>
                    </div>
                  )}
      ...
      {leaderboard.length > 0 ? (
        <div className="overflow-hidden">
          ...
          <div className="overflow-x-auto rounded-3xl border-4 border-slate-300 bg-white shadow-2xl">
            <table className="min-w-full divide-y divide-slate-200">
              <thead className="bg-slate-100">
                <tr>
                  <th className="px-4 py-4 text-center text-sm font-semibold uppercase tracking-wider text-slate-700 border-l border-slate-200 first:border-l-0">
                    Rang
                  </th>
                  <th className="px-4 py-4 text-center text-sm font-semibold uppercase tracking-wider text-slate-700 border-l border-slate-200 first:border-l-0">
                    Joueur
                ...
                {leaderboard.map((player, idx) => {
                  ...
                  return (
                    <tr key={player.user_id} className={rowClass}>
                      <td className="px-4 py-4 text-sm font-semibold text-slate-800 text-center border-l border-slate-200 first:border-l-0">
                        <RankBadge rank={player.rank} size="md" />
                      </td>
                      <td className="px-4 py-4 text-sm text-slate-800 text-center border-l border-slate-200 first:border-l-0">
                        <span className="font-semibold">{player.player_name}</span>
                      </td>
                      <td className="px-4 py-4 text-sm text-center border-l border-slate-200 first:border-l-0">
                        <TierBadge tier={tier} size="sm" />
                      </td>
                      <td className="px-4 py-4 text-sm text-center tabular-nums text-slate-900 border-l border-slate-200 first:border-l-0 font-semibold">
                        {player.points}
                      </td>
                      <td className="px-4 py-4 text-sm text-center tabular-nums border-l border-slate-200 first:border-l-0 font-semibold text-emerald-600">
                        {winRate}%
                      </td>
                      <td className="px-4 py-4 text-sm text-center tabular-nums text-emerald-700 bg-emerald-50 border-l border-slate-200 first:border-l-0 font-semibold">
                        {player.wins}
                      </td>
                      <td className="px-4 py-4 text-sm text-center tabular-nums text-rose-700 bg-rose-50 border-l border-slate-200 first:border-l-0 font-semibold">
                        {player.losses}
                      </td>
                      <td className="px-4 py-4 text-sm text-center tabular-nums text-slate-700 border-l border-slate-200 first:border-l-0 font-semibold">
                        {player.matches}
                      </td>
                    </tr>
                  );
        ...
        <div className="rounded-xl border border-white/40 ring-1 ring-white/10 bg-white/5 p-6 text-sm text-white/60">
