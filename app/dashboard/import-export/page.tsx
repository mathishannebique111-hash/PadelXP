      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="px-3 py-2 rounded bg-white/10 border border-white/10 hover:bg-white/15 transition-colors"
          >
          ...
          >
            {isImportingMembers ? "Import en cours..." : "Importer"}
          </button>
          <input
        ...
        {membersImportMessage && (
          <div
            className={`mt-4 rounded-lg border px-4 py-3 text-sm ${
              membersImportMessage.type === "success"
                ? "border-emerald-400/40 bg-emerald-500/15 text-emerald-100"
                : "border-red-500/40 bg-red-500/15 text-red-100"
            }`}
          >
            {membersImportMessage.text}
            {membersImportDetails && (
              <div className="mt-2 text-xs text-white/70">
                <div>Total lignes lues : {membersImportDetails.total}</div>
                <div>Ajouts : {membersImportDetails.inserted}</div>
                <div>Mises à jour : {membersImportDetails.updated}</div>
                <div>Ignorées : {membersImportDetails.skipped}</div>
              </div>
            )}
          </div>
        )}
        {membersImportErrors.length > 0 && (
          <div className="mt-3 rounded-lg border border-amber-400/30 bg-amber-500/10 px-4 py-3 text-xs text-amber-100">
            <div className="font-semibold mb-1">Lignes ignorées :</div>
            <ul className="space-y-1 list-disc list-inside">
              {membersImportErrors.slice(0, 5).map((err, idx) => (
                <li key={idx}>{err}</li>
              ))}
            </ul>
            {membersImportErrors.length > 5 && (
              <div className="mt-1 opacity-70">
                {membersImportErrors.length - 5} message(s) supplémentaire(s) non affiché(s)
              </div>
            )}
          </div>
        )}
